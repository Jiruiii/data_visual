        ...
        if mode == 'compare' and countries_param:
            ...
            for country_name in countries[:5]:
                ...
                if not country_df.empty:
                    yearly_counts = country_df.groupby(
            ...
            if len(yearly_stats) >= 2:
                ...
                trend = ((last_year_count - first_year_count) /
                         first_year_count) * 100 if first_year_count > 0 else 0

                first_year_loss = yearly_stats.iloc[0]['Loss']
                last_year_loss = yearly_stats.iloc[-1]['Loss']
                loss_trend = (
                    (last_year_loss - first_year_loss) /
                    first_year_loss) * 100 if first_year_loss > 0 else 0
            else:
                trend = 0
                loss_trend = 0
...
def get_sankey():
    ...
    try:
        ...
        # 取得篩選參數
        countries = request.args.get(
            'countries',
            '').split(',') if request.args.get('countries') else []
        attacks = request.args.get(
            'attacks', '').split(',') if request.args.get('attacks') else []
        industries = request.args.get(
            'industries',
            '').split(',') if request.args.get('industries') else []
        ...
        if not attacks:
            top_attacks = df['Attack Type'].value_counts().head(
                6).index.tolist()
        else:
            top_attacks = [a for a in attacks if a]

        if not industries:
            top_industries = df['Target Industry'].value_counts().head(
                8).index.tolist()
        else:
        ...
        # 連結 1: 國家 → 攻擊類型
        link1 = df_filtered.groupby(['Country', 'Attack Type'
                                     ]).size().reset_index(name='value')
        link1 = link1[link1['value'] > min_attacks]

        # 連結 2: 攻擊類型 → 目標產業
        link2 = df_filtered.groupby(['Attack Type', 'Target Industry'
                                     ]).size().reset_index(name='value')
        link2 = link2[link2['value'] > min_attacks]

        sources = link1['Country'].map(
            node_dict).tolist() + link2['Attack Type'].map(node_dict).tolist()
        targets = link1['Attack Type'].map(node_dict).tolist(
        ...
        return jsonify({
            ...
            ['#4ECDC4'] * len(top_attacks) + ['#95E1D3'] * len(top_industries),
            'sources':
            sources,
            'targets':
            targets,
...
def get_treemap():
    ...
    try:
        ...
        if level == 'both':
            # 第一層：產業統計
            industry_counts = df.groupby('Target Industry').size().reset_index(
                name='Count')
            industry_counts = industry_counts[industry_counts['Count']
                                              > 5]  # 過濾小數據
            ...
            # 第二層：產業下的攻擊類型
            industry_attack = df.groupby(['Target Industry', 'Attack Type'
            ...
            # 只顯示攻擊類型層級
            attack_counts = df.groupby('Attack Type').size().reset_index(
                name='Count')
        ...
        for i in range(len(labels)):
            if pd.notna(labels[i]) and pd.notna(parents[i]) and pd.notna(
                    values[i]):
                if values[i] > 0 or parents[i] == '':  # 保留根節點和有效數值
                    cleaned_data.append({
                        ...
                        str(parents[i]),
                        'value':
                        int(values[i]) if values[i] > 0 else 0
                    })
